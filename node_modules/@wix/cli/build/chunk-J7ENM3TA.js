import { createRequire as _createRequire } from 'node:module';
const require = _createRequire(import.meta.url);
import{a as U,b as Q,c as q,e as s,f as jI,g as MI,h as OI,i as Pg,l as Yg,q as PI,r as qI,s as _I,t as Xl,u as Yl,w as aI}from"./chunk-BG2UJUYU.js";import{$a as eI,$b as sl,C as Il,D as ll,Da as _,Db as X,Gb as Gl,I as Cl,J as ol,Jb as wg,Mb as bg,Na as CI,Ob as al,Ra as y,Rb as bI,Sb as Bl,Ta as J,Tb as nI,U as cl,Vb as iI,Xa as Fg,Xb as GI,Y as el,Ya as Zg,Za as oI,Zb as tl,_a as cI,_b as ml,aa as dl,ab as Jg,ac as Al,ba as Zl,bc as Wl,ca as bl,cb as il,cc as ul,dc as Vl,eb as dI,ec as rl,fa as eg,fb as $,ga as vg,hb as Bg,ja as nl,k as Kg,lb as gg,m as gI,mb as Sg,q as lg,qa as x,r as V,ra as M,rb as tg,sa as t,sb as ZI,t as Cg,ta as II,wa as dg,xa as lI,y as $I,z as gl,za as og}from"./chunk-H5MPJJNF.js";import{b as kC}from"./chunk-TVPGNQRX.js";import{e as pl,f as hl,g as yl}from"./chunk-5BCJKJOV.js";import{a as yg,b as Hg}from"./chunk-DUF2VEL4.js";import{a as Xg,b as qg,c as _g,d as DI,e as $g,g as Rg,h as Ng}from"./chunk-2OKEAQXU.js";import{a as f,d as EI,e as H,i as n}from"./chunk-UO4ZAUJK.js";n();var vC=H(U(),1);import TI from"node:process";n();var sg=H(U(),1),BI=H(qg(),1);import{dirname as UC}from"node:path";import{appendFileSync as QC,mkdirSync as LC}from"node:fs";n();var Hl=H(kC(),1),Rl=H(qg(),1);function mg(I){if(Hl.HttpClient.isHttpError(I))return I;let g=I instanceof Error?Rl.default.cause(I):null;return g?mg(g):null}n();function zg(I){return{request:{baseURL:I.config?.baseURL,url:I.config?.url,method:I.config?.method,headers:I.config?.headers,data:I.config?.data},response:{code:I.code,message:I.message,status:I.response?.status,headers:I.response?.headers,data:I.response?.data}}}function tI(I,g){try{let l=`----- CLI crash report at ${new Date().toISOString()}
`;if(g instanceof Error){l+=`Error Trace
`,l+=`${BI.default.fullStack(g)}
`,l+=`Error Info
`,l+=`${JSON.stringify(BI.default.info(g),null,2)}
`;let C=mg(g);C&&(l+=`Request Info
`,l+=`${JSON.stringify(zg(C),null,2)}
`)}LC(UC(I),{recursive:!0}),QC(I,l)}catch{}}var $c=(0,sg.createContext)(null);n();var Ug=H(U(),1);n();var c=H(U(),1),AI=H(Xg(),1);n();function kg(){for(var I=[],g=0;g<arguments.length;g++)I[g]=arguments[g]}function Nl(){return typeof WeakMap<"u"?new WeakMap:fC()}function fC(){return{add:kg,delete:kg,get:kg,set:kg,has:function(I){return!1}}}var xC=Object.prototype.hasOwnProperty,sI=function(I,g){return xC.call(I,g)};function mI(I,g){for(var l in g)sI(g,l)&&(I[l]=g[l]);return I}var TC=/^[ \t]*(?:\r\n|\r|\n)/,EC=/(?:\r\n|\r|\n)[ \t]*$/,jC=/^(?:[\r\n]|$)/,MC=/(?:\r\n|\r|\n)([ \t]*)(?:[^ \t\r\n]|$)/,OC=/^[ \t]*[\r\n][ \t\r\n]*$/;function Kl(I,g,l){var C=0,o=I[0].match(MC);o&&(C=o[1].length);var e="(\\r\\n|\\r|\\n).{0,"+C+"}",d=new RegExp(e,"g");g&&(I=I.slice(1));var Z=l.newline,G=l.trimLeadingNewline,i=l.trimTrailingNewline,a=typeof Z=="string",m=I.length,B=I.map(function(A,W){return A=A.replace(d,"$1"),W===0&&G&&(A=A.replace(TC,"")),W===m-1&&i&&(A=A.replace(EC,"")),a&&(A=A.replace(/\r\n|\n|\r/g,function(Y){return Z})),A});return B}function DC(I,g){for(var l="",C=0,o=I.length;C<o;C++)l+=I[C],C<o-1&&(l+=g[C]);return l}function PC(I){return sI(I,"raw")&&sI(I,"length")}function vl(I){var g=Nl(),l=Nl();function C(e){for(var d=[],Z=1;Z<arguments.length;Z++)d[Z-1]=arguments[Z];if(PC(e)){var G=e,i=(d[0]===C||d[0]===v)&&OC.test(G[0])&&jC.test(G[1]),a=i?l:g,m=a.get(G);if(m||(m=Kl(G,i,I),a.set(G,m)),d.length===0)return m[0];var B=DC(m,i?d.slice(1):d);return B}else return vl(mI(mI({},I),e||{}))}var o=mI(C,{string:function(e){return Kl([e],!1,I)[0]}});return o}var v=vl({trimLeadingNewline:!0,trimTrailingNewline:!0});if(typeof module<"u")try{module.exports=v,Object.defineProperty(v,"__esModule",{value:!0}),v.default=v,v.outdent=v}catch{}n();var Ig=H(U(),1);var b=({message:I,explanation:g,hint:l})=>Ig.default.createElement(Q,{flexDirection:"column"},Ig.default.createElement(Pg,{type:"error"},I),g&&Ig.default.createElement(Pg,{type:"warning"},g),l&&Ig.default.createElement(Q,{paddingTop:1,columnGap:1},Ig.default.createElement(jI,null),typeof l=="string"?Ig.default.createElement(s,null,l):l)),r=({message:I,explanation:g,hint:l})=>Ig.default.createElement(b,{message:I,explanation:g,hint:l??"This is probably a temporary system error and has been logged. Try again shortly."});function Jl(I){return(0,AI.match)(I,{UnknownFailure:({error:g})=>()=>c.default.createElement(c.default.Fragment,null,c.default.createElement(s,{skin:"error"},"Oh no, seems like the CLI encountered an unexpected error:"),g instanceof Error?c.default.createElement(s,{skin:"error"},g.message):c.default.createElement(s,null,JSON.stringify(g))),FailedToGetDeviceCode:()=>()=>c.default.createElement(r,{message:"An error occured while logging in: Failed to issue a login device code"}),FailedToGetAuthToken:()=>()=>c.default.createElement(r,{message:"An error occured while logging in: Failed to issue refresh token"}),FailedToRefreshTokenForSite:()=>()=>c.default.createElement(r,{message:"An error occured while logging in: Failed to issue refresh token for site"}),FailedToRenewAuthToken:()=>()=>c.default.createElement(r,{message:"Failed to renew access token."}),FailedToRenewAuthTokenSiteNotFound:({siteId:g})=>()=>c.default.createElement(b,{message:`Authenticate failed, your configured development site (${g}) could not be accessed`,hint:"Make sure the site exists and you still have access to it"}),FailedToRenewAuthTokenRefreshTokenRevoked:()=>()=>c.default.createElement(b,{message:"Failed to renew access token: refresh token has been revoked",hint:"Please login again to get a new refresh token"}),FailedToRevokeRefreshToken:({email:g})=>()=>c.default.createElement(r,{message:"An error occured while trying to logout: Failed to revoke refresh token!",explanation:`Wix CLI is still logged in to account ${g}`}),FailedToFetchPackageJson:()=>()=>c.default.createElement(r,{message:"Failed to fetch package data."}),LatestVersionOfCreateAppRequired:({currentVersion:g,latestVersion:l,updateCommand:C})=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"Generating a Wix App is only supported using the latest version (",l,") of ",c.default.createElement(s,{bold:!0},"@wix/create-app")," (current:"," ",g,")"),hint:c.default.createElement(s,null,"Make sure you are using the following command:",c.default.createElement(q,null),c.default.createElement(s,{bold:!0},C),c.default.createElement(q,null),"If the problem persists, try clearing your package manager's cache")}),EmptyDirectoryRequiredAppGeneration:({folder:g})=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"Generating a Wix App requires an empty directory but"," ",c.default.createElement(s,{bold:!0},g)," already exists and isn't empty"),hint:c.default.createElement(s,null,"Choose a different project name or delete the existing"," ",c.default.createElement(s,{bold:!0},g)," folder")}),InvalidNewPackageName:({packageName:g,validations:l})=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"Generating a Wix App requires a valid npm package name for the generated ",c.default.createElement(s,{bold:!0},"package.json")," ",c.default.createElement(q,null),"The name ",c.default.createElement(s,{bold:!0},g)," failed the following validations:",c.default.createElement(q,null),l.map((C,o)=>c.default.createElement(s,{key:o},c.default.createElement(q,null),"- ",C))),hint:"Choose a different project name that follows the validations above"}),SiteSelectorTimedout:()=>()=>c.default.createElement(b,{message:"Waiting for application installation timed out."}),TypeErrorInvalidUrl:()=>()=>c.default.createElement(b,{message:"Invalid assets URL."}),UnsupportedPackageManager:({packageManagerName:g})=>()=>c.default.createElement(b,{message:"Generating a Wix App requires the usage of a supported package manager",hint:`Expected Yarn or NPM but ${g?`got ${g}`:"failed to detect a package manager"}`}),UnsupportedPackageManagerWorkspaces:({packageManagerName:g})=>()=>c.default.createElement(b,{message:`Generating a Wix App inside ${g} workspaces is not supported`}),FailedToGetDeveloperApps:()=>()=>c.default.createElement(r,{message:"Failed to get the list of your applications"}),FailedToCreateDevCenterApp:()=>()=>c.default.createElement(r,{message:"Failed to register your application in Wix Dev Center"}),FailedToCreateTunnel:()=>()=>c.default.createElement(b,{message:"Failed to create tunnel"}),FailedToDeleteTunnel:()=>()=>c.default.createElement(b,{message:"Failed to delete tunnel"}),SitePermissionDenied:({siteId:g})=>()=>c.default.createElement(b,{message:`You don't have permissions to site ${g}`}),AppPermissionDenied:({appId:g})=>()=>c.default.createElement(b,{message:`You don't have permissions to application ${g}`}),FailedToUpdateTelemetryConfig:()=>()=>c.default.createElement(b,{message:"Failed to change telemetry preferences"}),FailedToInstallPackage:({packageName:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Failed to install npm package ${g}. Cause: ${C}`})},FailedToResolveNpmDependencies:()=>()=>c.default.createElement(r,{message:"Failed to resolve npm dependencies"}),FailedToInstallPackages:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to install npm packages. Cause: ${l}`})},FailedMigrationToAnyNpm:()=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"Could not start the migration process"),hint:c.default.createElement(s,null,"Make sure that your package.json does not have packages installed outside of dev dependencies")}),DeprecatedVersion:({currentVersion:g})=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,`Your version of the Wix CLI (${g}) is`,c.default.createElement(s,{bold:!0}," deprecated"),"."),hint:"Please update to the latest version to continue"}),FailedToUninstallPackage:({packageName:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Failed to uninstall npm package ${g}. Cause: ${C}`})},FailedToSyncToRevision:({revision:g})=>()=>c.default.createElement(b,{message:`Failed to sync the local project to revision ${g}`}),FailedToCreateVersion:()=>()=>c.default.createElement(b,{message:"Wix CLI failed to create version for your app"}),FailedToReadCache:({path:g})=>()=>c.default.createElement(b,{message:`Failed to read cache file at ${g}.`}),FailedToWriteCache:({path:g})=>()=>c.default.createElement(b,{message:`Failed to write cache file at ${g}.`}),UploadApplicationFailed:()=>()=>c.default.createElement(b,{message:"Failed to upload the application"}),BuiltProjectNeeded:()=>()=>c.default.createElement(b,{message:"Can not find build output, please execute `wix app build` to build your app before proceeding"}),WixUninstallMissingPackageName:()=>()=>c.default.createElement(b,{message:"Please specify a package name"}),FailedToReadJson:({filePath:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Failed to read ${g}. Cause: ${C}`})},InvalidJsonFile:({filePath:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Invalid JSON file located at ${g}. Cause: ${C}`})},FailedToWriteWixLock:()=>()=>c.default.createElement(b,{message:"Failed to write 'wix.lock'"}),InvalidVeloConfigJson:({configFilename:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Invalid ${g}, make sure it's configured correctly. Cause: ${C}`})},DashboardPageConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Dashboard page config not found at ${g}`}),EmbeddedScriptConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Embedded script config not found at ${g}`}),CustomElementConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Custom element config not found at ${g}`}),NoExtensionsFound:()=>()=>c.default.createElement(b,{message:"Your project doesn't seem to have any extensions.",explanation:"Extensions are the building blocks of your app.",hint:"Make sure it is configured properly, or read our documentation (\u200Bhttps://wix.to/b7o4Ml0) to start developing your first extension."}),NoPagesDirectoryFound:({srcDir:g})=>()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"No ",c.default.createElement(s,{bold:!0},"pages")," directory was found under"," ",c.default.createElement(s,{bold:!0},g),"."),hint:c.default.createElement(s,null,"Consider re-cloning the project repository or creating a"," ",c.default.createElement(s,{bold:!0},"pages")," directory under ",c.default.createElement(s,{bold:!0},g),".")}),DashboardPluginConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Dashboard plugin config not found at ${g}`}),DashboardMenuPluginConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Dashboard menu plugin config not found at ${g}`}),ServicePluginConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Service plugin config not found at ${g}`}),DashboardModalConfigNotFound:({configPath:g})=>()=>c.default.createElement(b,{message:`Dashboard modal config not found at ${g}`}),InvalidConfigSchemaError:({configPath:g,zodError:l})=>()=>c.default.createElement(b,{message:v`
            Invalid configuration found at \`${g}\`:
            The following errors were found:
            ${l.errors.map(Fl).join(`
`)}`,hint:"Fix the errors above for your configurations to be applied successfully."}),InvalidParamsOverrideSchemaError:({paramsOverridePath:g,zodError:l})=>()=>c.default.createElement(b,{message:v`
            Invalid params override found at \`${g}\`:
            The following errors were found:
            ${l.errors.map(Fl).join(`
`)}`,hint:"Fix the errors above for your params override to be applied successfully."}),PermissionDenied:()=>()=>c.default.createElement(b,{message:"Forbidden operation. Missing permissions"}),FailedToReadProjectFiles:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to read source files: make sure you have an 'src' folder and that it's readable. Cause: ${l}`})},FailedToCreateVersionForSubmittedApp:()=>()=>c.default.createElement(b,{message:"Cannot create a new application version because the app has an active marketplace submission",hint:"You will be able continue creating versions when your submission is approved or retracted"}),GridAppFilesMaxLengthExcceeded:({maxSizeViolationsData:g,filePaths:l})=>{let C="Found one or more project files that are bigger than the allowed limit";return g?.files?.length&&(C+=` (${g.expectedSize}KB): ${g.files.map(o=>`${l[o.fileIndex]} (${o.receivedSize}KB)`).join(", ")}`),()=>c.default.createElement(b,{message:C})},GridAppFailedToUpdateFiles:()=>()=>c.default.createElement(b,{message:v`
            Failed to update files.
            Failed to sync changes from local files to the Dev Editor
            Still watching local files and will continue to sync changes...
          `}),FailedToWatchFiles:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to watch files. Cause: ${l}`})},FailedToCreateGridApp:({error:g})=>()=>c.default.createElement(b,{message:"Failed to create an isolated environment.",explanation:g}),DeviceCodeTimedOut:()=>()=>c.default.createElement(b,{message:"The device code has not been approved in the time allocated, please start the login process again."}),DuplicateComponentIdsError:({issues:g})=>{let l=({issue:o})=>{let e=d=>(0,AI.match)(d,{page:()=>"page",plugin:()=>"plugin","menu-plugin":()=>"menu plugin","sidebar-category":()=>"sidebar category","embedded-script":()=>"embedded script","custom-element":()=>"custom element",modal:()=>"modal",topology:()=>"topology"});return c.default.createElement(Q,{flexDirection:"column"},c.default.createElement(s,null,c.default.createElement(s,{bold:!0},"Duplicate ID: "),o.id),c.default.createElement(s,{bold:!0},"Extensions:"),o.components.map(({name:d,path:Z,type:G})=>c.default.createElement(s,{key:Z},"[",e(G),"] ",d," (",Z,")")),c.default.createElement(s,null,c.default.createElement(s,{bold:!0},"Possible GUID: "),o.suggestedId))},C=c.default.createElement(Q,{flexDirection:"column",rowGap:1},c.default.createElement(s,null,"You can use the provided suggested GUIDs as the ID of the newly created extension."),g.map(o=>c.default.createElement(l,{issue:o,key:o.id})));return()=>c.default.createElement(b,{message:"Found multiple extensions with the same id, this might happen if you copy paste folders.",hint:C})},InvalidResponseData:()=>()=>c.default.createElement(r,{message:"Got unexpected response from server."}),InvalidCreateAppResponse:()=>()=>c.default.createElement(b,{message:"`createApp` returned an invalid response."}),InvalidDeployAppResponse:()=>()=>c.default.createElement(b,{message:"Got unexpected response from server. Make sure you are running a supported version of the Velo CLI."}),InvalidGetDeploymentResponse:()=>()=>c.default.createElement(b,{message:"Got unexpected response from server. Please try again later.."}),FailedToFetchRegisteredEvents:()=>()=>c.default.createElement(b,{message:"Failed to fetch registered events from backend server."}),FailedToGetPagesDiff:()=>()=>c.default.createElement(b,{message:"Failed to request pages diff."}),InvalidViteBuildOutput:()=>()=>c.default.createElement(b,{message:"Invalid vite build output"}),ViteModuleBundleNotFound:()=>()=>c.default.createElement(b,{message:"Could not find module bundle in vite build output"}),ViteFileNotFoundInOutput:({fileName:g})=>()=>c.default.createElement(b,{message:`Could not find ${g} in vite build output`}),FailedToCreatePresignedUrl:()=>()=>c.default.createElement(b,{message:"Failed to create a presigned upload url"}),FailedToCreateBackendDeployment:()=>()=>c.default.createElement(b,{message:"Failed to create backend deployment"}),FailedToUpdateBackendDeployment:()=>()=>c.default.createElement(b,{message:"Failed to update backend deployment"}),FailedToGetBackendDeployment:()=>()=>c.default.createElement(b,{message:"Failed to get backend deployment"}),FailedToDeployBackend:()=>()=>c.default.createElement(b,{message:"Failed to deploy backend"}),FailedToUploadApplicationFiles:()=>()=>c.default.createElement(b,{message:"Failed to upload the application files"}),FailedToVerifyDeployment:()=>()=>c.default.createElement(b,{message:"Deployment was not finished within the desired timeframe, please try again later."}),FailedToGetUserInfo:()=>()=>c.default.createElement(b,{message:"Failed to get user information."}),FailedToGetMyAccount:()=>()=>c.default.createElement(r,{message:"Failed to get account information."}),FailedToCreateDevelopmentSite:()=>()=>c.default.createElement(b,{message:"Failed to create a Development Site"}),FailedToGetDevelopmentSites:()=>()=>c.default.createElement(b,{message:"Failed to request development sites."}),FailedToGetDevelopmentSitesLimit:()=>()=>c.default.createElement(b,{message:"Failed to request development sites limit."}),FailedToGetRequiredApps:()=>()=>c.default.createElement(b,{message:"Failed to get app required apps."}),FailedToGetMarketListing:()=>()=>c.default.createElement(b,{message:"Failed to get market listing."}),FailedToAddRequiredApp:()=>()=>c.default.createElement(b,{message:"Failed to add required app."}),FailedToQuerySites:()=>()=>c.default.createElement(b,{message:"Failed to request sites list."}),FailedToGetSite:()=>()=>c.default.createElement(b,{message:"Failed to get information about your configured Development Site"}),FailedToDeploySite:()=>()=>c.default.createElement(b,{message:"Failed to deploy site."}),FailedToPublishDeployment:()=>()=>c.default.createElement(r,{message:"Failed to publish site."}),FailedToSaveDevelopmentSite:({path:g})=>()=>c.default.createElement(b,{message:"Failed to save the current development site.",hint:`Make sure the \`${g}\` file has proper permissions and is accessible`}),FailedToBuildApp:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:c.default.createElement(c.default.Fragment,null,"Failed to build the application.",c.default.createElement(q,null),l),hint:"Please check the build output for more information"})},FailedToCheckIfAppInstalled:()=>()=>c.default.createElement(b,{message:"Failed to fetch site installation details"}),FailedToInitializeGitRepo:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to initialize git repo. Cause: ${l}`})},FailedToInstallDependencies:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to install dependencies. Cause: ${l}`})},FailedToCommitChanges:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to commit changes. Cause: ${l}`})},FailedToDeleteAuthData:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to delete auth data: ${l}`})},FailedToDeleteLegacyAuthData:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to delete legacy auth data: ${l}`})},FailedToSaveAuthData:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to save auth data: ${l}`})},FailedToSaveApiKeyAuthData:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to save api key auth data: ${l}`})},FailedToWriteJson:({filePath:g,cause:l})=>{let C=R(l);return()=>c.default.createElement(b,{message:`Failed to write JSON at path: ${g}. Cause: ${C}`})},TemplatePathNotFound:({templatePath:g})=>()=>c.default.createElement(b,{message:`Template path not found: ${g}`,hint:"Make sure the template path exists in the remote repository"}),FailedToCloneRepo:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Failed to clone repo: ${l}`,hint:"Make sure the Git URL is correct and that your local Git is configured properly to access the remote repository"})},InvalidGitUrl:({url:g})=>()=>c.default.createElement(b,{message:`Invalid Git URL: ${g}`,hint:"Make sure the Git URL is in the correct format"}),AuthenticationRequired:()=>()=>c.default.createElement(b,{message:"You have to already be logged in to run this command."}),SiteConfiguredRequired:()=>()=>c.default.createElement(b,{message:"You must configure the development site before running this command, or provide a site-id in the command options."}),FailedAddingPackageToWorkspace:()=>()=>c.default.createElement(b,{message:"Could not add package to the workspaces configuration. Please add it as instructed by your package manager.",hint:c.default.createElement(s,null,"Full error can be found in ",c.default.createElement(s,{bold:!0},"wix-error.log"))}),FailedToGenerateSelfsignedCertificate:()=>()=>c.default.createElement(b,{message:"Failed to generate a Selfsigned certificate for development."}),NonInteractiveTerminal:()=>()=>c.default.createElement(b,{message:"This command is not supported in non-interactive terminals. Please run the command in an interactive terminal (TTY)."}),FailedToResolveViteOrigin:()=>()=>c.default.createElement(b,{message:"Failed to resolve the base url for CSS asset urls"}),FailedToCreateAppRelease:()=>()=>c.default.createElement(r,{message:"Failed to create an app configuration in dev center."}),FailedToUpdateAppRelease:()=>()=>c.default.createElement(r,{message:"Failed to update an app configuration in dev center."}),FailedToCreateComponents:()=>()=>c.default.createElement(r,{message:"Failed to create components in dev center."}),FailedToCheckDeprecatedStatus:()=>()=>c.default.createElement(r,{message:"Failed to check cli version deprecated status."}),FailedToGetResolveNpmDependenciesResult:()=>()=>c.default.createElement(r,{message:"Failed to resolve Wix's lockfile."}),CliAppVersionMismatch:({cliVersion:g,cliAppVersion:l})=>()=>c.default.createElement(b,{message:c.default.createElement(s,null,"The versions of ",c.default.createElement(s,{bold:!0},"@wix/cli")," (",g,") and"," ",c.default.createElement(s,{bold:!0},"@wix/cli-app")," (",l,") dependencies do not match"),hint:c.default.createElement(s,null,"Align their versions in your ",c.default.createElement(s,{bold:!0},"package.json")," and re-install dependencies")}),FailedToGetAppValidity:()=>()=>c.default.createElement(r,{message:"Failed to get validation status for app version from dev center"}),InvalidDashboardPageRoute:({route:g,errorMessage:l})=>()=>c.default.createElement(b,{message:`Invalid dashboard page route: ${g}`,hint:l}),InvalidEmbeddedScriptFolder:({path:g,errorMessage:l})=>()=>c.default.createElement(b,{message:`Invalid embedded script folder: ${g}`,hint:l}),InvalidEmbeddedScriptTemplate:({templateFilename:g,errorMessage:l})=>()=>c.default.createElement(b,{message:`Invalid embedded script template in ${g}`,hint:l}),InvalidCustomElementTagName:({tagName:g,errorMessage:l})=>()=>c.default.createElement(b,{message:c.default.createElement(s,null,"We use name of folder as tagName for your custom element. Unfortunately, ",c.default.createElement(s,{bold:!0},g)," is invalid tagName. Reason: ",l),hint:c.default.createElement(s,null,"You can find rules for tagName here:"," ",c.default.createElement(s,{bold:!0},"https://developer.mozilla.org/en-US/docs/Web/API/CustomElementRegistry/define#valid_custom_element_names"))}),InvalidSiteFrontendPackageVersion:({packageName:g,versionRange:l})=>()=>c.default.createElement(b,{message:c.default.createElement(s,null,"Package ",c.default.createElement(s,{bold:!0},g)," has an invalid version range ",c.default.createElement(s,{bold:!0},l)," in your"," ",c.default.createElement(s,{bold:!0},"package.json")," file."),hint:c.default.createElement(s,null,"The version range must follow"," ",c.default.createElement(s,{bold:!0},"semantic versioning")," rules, and it must contain a major version.",c.default.createElement(q,null),"Read more about it here:"," ",c.default.createElement(Yg,{url:"https://docs.npmjs.com/about-semantic-versioning"}))}),FailedToCreateSiteOverride:()=>()=>c.default.createElement(r,{message:"Failed to create site override configuration"}),FailedToGetPipelineErrorInfo:()=>()=>c.default.createElement(r,{message:"Failed to get error details"}),FailedToGetDeploymentStatus:()=>()=>c.default.createElement(r,{message:"Failed to get deployment status"}),NetworkError:({cause:g})=>{let l=R(g);return()=>c.default.createElement(b,{message:`Network error occurred. Cause: ${l}`,hint:"Make sure you have an active internet connection and try again shortly."})},FailedToGenerateAppMarketQueryParam:()=>()=>c.default.createElement(r,{message:"Failed to get generate appMarket query parameter"})})}function R(I){return I instanceof Error?I.message:JSON.stringify(I)}function Fl(I){let g=I.path.join(".");return g?`- \`${g}\`: ${I.message}`:`- ${I.message}`}var WI=({error:I,systemErrorOverride:g})=>(0,Ug.useMemo)(()=>{let l=Ng(I);return Rg(l)&&g?g:(0,Ug.createElement)(Jl(l.cliCode))},[I,g]);n();var ig=H(Xg(),1);n();n();import*as Sl from"os";import{TextEncoder as qC}from"util";var Ag=class extends Gl{constructor(g){al(g,"node"),g.transportOptions={textEncoder:new qC,...g.transportOptions};let l={...g,platform:"node",runtime:{name:"node",version:global.process.version},serverName:g.serverName||global.process.env.SENTRY_NAME||Sl.hostname()};super(l)}};n();import*as go from"http";import*as Io from"https";import{Readable as lo}from"stream";import{URL as kl}from"url";import{createGzip as Co}from"zlib";n();import*as O from"net";import*as VI from"tls";import{URL as _C}from"url";n();import*as uI from"http";var T=Symbol("AgentBaseInternalState"),Qg=class extends uI.Agent{constructor(g){super(g),this[T]={}}isSecureEndpoint(g){if(g){if(typeof g.secureEndpoint=="boolean")return g.secureEndpoint;if(typeof g.protocol=="string")return g.protocol==="https:"}let{stack:l}=new Error;return typeof l!="string"?!1:l.split(`
`).some(C=>C.indexOf("(https.js:")!==-1||C.indexOf("node:https:")!==-1)}createSocket(g,l,C){let o={...l,secureEndpoint:this.isSecureEndpoint(l)};Promise.resolve().then(()=>this.connect(g,o)).then(e=>{if(e instanceof uI.Agent)return e.addRequest(g,o);this[T].currentSocket=e,super.createSocket(g,l,C)},C)}createConnection(){let g=this[T].currentSocket;if(this[T].currentSocket=void 0,!g)throw new Error("No socket was returned in the `connect()` function");return g}get defaultPort(){return M(this[T].defaultPort,()=>this.protocol==="https:"?443:80)}set defaultPort(g){this[T]&&(this[T].defaultPort=g)}get protocol(){return M(this[T].protocol,()=>this.isSecureEndpoint()?"https:":"http:")}set protocol(g){this[T]&&(this[T].protocol=g)}};n();function Lg(...I){V.log("[https-proxy-agent:parse-proxy-response]",...I)}function wl(I){return new Promise((g,l)=>{let C=0,o=[];function e(){let a=I.read();a?i(a):I.once("readable",e)}function d(){I.removeListener("end",Z),I.removeListener("error",G),I.removeListener("readable",e)}function Z(){d(),Lg("onend"),l(new Error("Proxy connection ended before receiving CONNECT response"))}function G(a){d(),Lg("onerror %o",a),l(a)}function i(a){o.push(a),C+=a.length;let m=Buffer.concat(o,C),B=m.indexOf(`\r
\r
`);if(B===-1){Lg("have not received end of HTTP headers yet..."),e();return}let A=m.slice(0,B).toString("ascii").split(`\r
`),W=A.shift();if(!W)return I.destroy(),l(new Error("No header received from proxy CONNECT response"));let Y=W.split(" "),p=+Y[1],w=Y.slice(2).join(" "),z={};for(let k of A){if(!k)continue;let D=k.indexOf(":");if(D===-1)return I.destroy(),l(new Error(`Invalid header from proxy CONNECT response: "${k}"`));let F=k.slice(0,D).toLowerCase(),u=k.slice(D+1).trimStart(),h=z[F];typeof h=="string"?z[F]=[h,u]:Array.isArray(h)?h.push(u):z[F]=u}Lg("got proxy server response: %o %o",W,z),d(),g({connect:{statusCode:p,statusText:w,headers:z},buffered:m})}I.on("error",G),I.on("end",Z),e()})}function Wg(...I){V.log("[https-proxy-agent]",...I)}var ug=class extends Qg{static __initStatic(){this.protocols=["http","https"]}constructor(g,l){super(l),this.options={},this.proxy=typeof g=="string"?new _C(g):g,this.proxyHeaders=M(t([l,"optionalAccess",e=>e.headers]),()=>({})),Wg("Creating new HttpsProxyAgent instance: %o",this.proxy.href);let C=(this.proxy.hostname||this.proxy.host).replace(/^\[|\]$/g,""),o=this.proxy.port?parseInt(this.proxy.port,10):this.proxy.protocol==="https:"?443:80;this.connectOpts={ALPNProtocols:["http/1.1"],...l?zl(l,"headers"):null,host:C,port:o}}async connect(g,l){let{proxy:C}=this;if(!l.host)throw new TypeError('No "host" provided');let o;if(C.protocol==="https:"){Wg("Creating `tls.Socket`: %o",this.connectOpts);let B=this.connectOpts.servername||this.connectOpts.host;o=VI.connect({...this.connectOpts,servername:B&&O.isIP(B)?void 0:B})}else Wg("Creating `net.Socket`: %o",this.connectOpts),o=O.connect(this.connectOpts);let e=typeof this.proxyHeaders=="function"?this.proxyHeaders():{...this.proxyHeaders},d=O.isIPv6(l.host)?`[${l.host}]`:l.host,Z=`CONNECT ${d}:${l.port} HTTP/1.1\r
`;if(C.username||C.password){let B=`${decodeURIComponent(C.username)}:${decodeURIComponent(C.password)}`;e["Proxy-Authorization"]=`Basic ${Buffer.from(B).toString("base64")}`}e.Host=`${d}:${l.port}`,e["Proxy-Connection"]||(e["Proxy-Connection"]=this.keepAlive?"Keep-Alive":"close");for(let B of Object.keys(e))Z+=`${B}: ${e[B]}\r
`;let G=wl(o);o.write(`${Z}\r
`);let{connect:i,buffered:a}=await G;if(g.emit("proxyConnect",i),this.emit("proxyConnect",i,g),i.statusCode===200){if(g.once("socket",$C),l.secureEndpoint){Wg("Upgrading socket connection to TLS");let B=l.servername||l.host;return VI.connect({...zl(l,"host","path","port"),socket:o,servername:O.isIP(B)?void 0:B})}return o}o.destroy();let m=new O.Socket({writable:!1});return m.readable=!0,g.once("socket",B=>{Wg("Replaying proxy buffer for failed request"),B.push(a),B.push(null)}),m}};ug.__initStatic();function $C(I){I.resume()}function zl(I,...g){let l={},C;for(C in I)g.includes(C)||(l[C]=I[C]);return l}var oo=1024*32;function co(I){return new lo({read(){this.push(I),this.push(null)}})}function fg(I){let g;try{g=new kl(I.url)}catch{return lg(()=>{console.warn("[@sentry/node]: Invalid dsn or tunnel option, will not send any events. The tunnel option must be a full URL when used.")}),wg(I,()=>Promise.resolve({}))}let l=g.protocol==="https:",C=eo(g,I.proxy||(l?process.env.https_proxy:void 0)||process.env.http_proxy),o=l?Io:go,e=I.keepAlive===void 0?!1:I.keepAlive,d=C?new ug(C):new o.Agent({keepAlive:e,maxSockets:30,timeout:2e3}),Z=Zo(I,M(I.httpModule,()=>o),d);return wg(I,Z)}function eo(I,g){let{no_proxy:l}=process.env;if(!(l&&l.split(",").some(o=>I.host.endsWith(o)||I.hostname.endsWith(o))))return g}function Zo(I,g,l){let{hostname:C,pathname:o,port:e,protocol:d,search:Z}=new kl(I.url);return function(i){return new Promise((a,m)=>{let B=co(i.body),A={...I.headers};i.body.length>oo&&(A["content-encoding"]="gzip",B=B.pipe(Co()));let W=g.request({method:"POST",agent:l,headers:A,hostname:C,path:`${o}${Z}`,port:e,protocol:d,ca:I.caCerts},Y=>{Y.on("data",()=>{}),Y.on("end",()=>{}),Y.setEncoding("utf8");let p=M(Y.headers["retry-after"],()=>null),w=M(Y.headers["x-sentry-rate-limits"],()=>null);a({statusCode:Y.statusCode,headers:{"retry-after":p,"x-sentry-rate-limits":Array.isArray(w)?w[0]:w}})});W.on("error",m),B.pipe(W)})}}n();n();var S=Cl(process.versions.node);n();import*as Ul from"util";var Ql="Console",bo=()=>({name:Ql,setupOnce(){},setup(I){ll(({args:g,level:l})=>{y()===I&&_({category:"console",level:bl(l),message:Ul.format.apply(void 0,g)},{input:[...g],level:l})})}}),rI=bo,Ll=X(Ql,rI);n();import{execFile as no}from"child_process";import{readFile as io,readdir as Go}from"fs";import*as N from"os";import{join as ao}from"path";import{promisify as xl}from"util";var Bo=xl(io),to=xl(Go),Tl="Context",mo=(I={})=>{let g,l={app:!0,os:!0,device:!0,culture:!0,cloudResource:!0,...I};async function C(e){g===void 0&&(g=o());let d=so(await g);return e.contexts={...e.contexts,app:{...d.app,...t([e,"access",Z=>Z.contexts,"optionalAccess",Z=>Z.app])},os:{...d.os,...t([e,"access",Z=>Z.contexts,"optionalAccess",Z=>Z.os])},device:{...d.device,...t([e,"access",Z=>Z.contexts,"optionalAccess",Z=>Z.device])},culture:{...d.culture,...t([e,"access",Z=>Z.contexts,"optionalAccess",Z=>Z.culture])},cloud_resource:{...d.cloud_resource,...t([e,"access",Z=>Z.contexts,"optionalAccess",Z=>Z.cloud_resource])}},e}async function o(){let e={};if(l.os&&(e.os=await Ao()),l.app&&(e.app=uo()),l.device&&(e.device=Vo(l.device)),l.culture){let d=Wo();d&&(e.culture=d)}return l.cloudResource&&(e.cloud_resource=Yo()),e}return{name:Tl,setupOnce(){},processEvent(e){return C(e)}}},pI=mo,El=X(Tl,pI);function so(I){return t([I,"optionalAccess",g=>g.app,"optionalAccess",g=>g.app_memory])&&(I.app.app_memory=process.memoryUsage().rss),t([I,"optionalAccess",g=>g.device,"optionalAccess",g=>g.free_memory])&&(I.device.free_memory=N.freemem()),I}async function Ao(){let I=N.platform();switch(I){case"darwin":return yo();case"linux":return Xo();default:return{name:ro[I]||I,version:N.release()}}}function Wo(){try{if(typeof process.versions.icu!="string")return;let I=new Date(9e8);if(new Intl.DateTimeFormat("es",{month:"long"}).format(I)==="enero"){let l=Intl.DateTimeFormat().resolvedOptions();return{locale:l.locale,timezone:l.timeZone}}}catch{}}function uo(){let I=process.memoryUsage().rss;return{app_start_time:new Date(Date.now()-process.uptime()*1e3).toISOString(),app_memory:I}}function Vo(I){let g={},l;try{l=N.uptime&&N.uptime()}catch{}if(typeof l=="number"&&(g.boot_time=new Date(Date.now()-l*1e3).toISOString()),g.arch=N.arch(),(I===!0||I.memory)&&(g.memory_size=N.totalmem(),g.free_memory=N.freemem()),I===!0||I.cpu){let C=N.cpus();if(C&&C.length){let o=C[0];g.processor_count=C.length,g.cpu_description=o.model,g.processor_frequency=o.speed}}return g}var ro={aix:"IBM AIX",freebsd:"FreeBSD",openbsd:"OpenBSD",sunos:"SunOS",win32:"Windows"},po=[{name:"fedora-release",distros:["Fedora"]},{name:"redhat-release",distros:["Red Hat Linux","Centos"]},{name:"redhat_version",distros:["Red Hat Linux"]},{name:"SuSE-release",distros:["SUSE Linux"]},{name:"lsb-release",distros:["Ubuntu Linux","Arch Linux"]},{name:"debian_version",distros:["Debian"]},{name:"debian_release",distros:["Debian"]},{name:"arch-release",distros:["Arch Linux"]},{name:"gentoo-release",distros:["Gentoo Linux"]},{name:"novell-release",distros:["SUSE Linux"]},{name:"alpine-release",distros:["Alpine Linux"]}],ho={alpine:I=>I,arch:I=>E(/distrib_release=(.*)/,I),centos:I=>E(/release ([^ ]+)/,I),debian:I=>I,fedora:I=>E(/release (..)/,I),mint:I=>E(/distrib_release=(.*)/,I),red:I=>E(/release ([^ ]+)/,I),suse:I=>E(/VERSION = (.*)\n/,I),ubuntu:I=>E(/distrib_release=(.*)/,I)};function E(I,g){let l=I.exec(g);return l?l[1]:void 0}async function yo(){let I={kernel_version:N.release(),name:"Mac OS X",version:`10.${Number(N.release().split(".")[0])-4}`};try{let g=await new Promise((l,C)=>{no("/usr/bin/sw_vers",(o,e)=>{if(o){C(o);return}l(e)})});I.name=E(/^ProductName:\s+(.*)$/m,g),I.version=E(/^ProductVersion:\s+(.*)$/m,g),I.build=E(/^BuildVersion:\s+(.*)$/m,g)}catch{}return I}function fl(I){return I.split(" ")[0].toLowerCase()}async function Xo(){let I={kernel_version:N.release(),name:"Linux"};try{let g=await to("/etc"),l=po.find(Z=>g.includes(Z.name));if(!l)return I;let C=ao("/etc",l.name),o=(await Bo(C,{encoding:"utf-8"})).toLowerCase(),{distros:e}=l;I.name=e.find(Z=>o.indexOf(fl(Z))>=0)||e[0];let d=fl(I.name);I.version=ho[d](o)}catch{}return I}function Yo(){return process.env.VERCEL?{"cloud.provider":"vercel","cloud.region":process.env.VERCEL_REGION}:process.env.AWS_REGION?{"cloud.provider":"aws","cloud.region":process.env.AWS_REGION,"cloud.platform":process.env.AWS_EXECUTION_ENV}:process.env.GCP_PROJECT?{"cloud.provider":"gcp"}:process.env.ALIYUN_REGION_ID?{"cloud.provider":"alibaba_cloud","cloud.region":process.env.ALIYUN_REGION_ID}:process.env.WEBSITE_SITE_NAME&&process.env.REGION_NAME?{"cloud.provider":"azure","cloud.region":process.env.REGION_NAME}:process.env.IBM_CLOUD_REGION?{"cloud.provider":"ibm_cloud","cloud.region":process.env.IBM_CLOUD_REGION}:process.env.TENCENTCLOUD_REGION?{"cloud.provider":"tencent_cloud","cloud.region":process.env.TENCENTCLOUD_REGION,"cloud.account.id":process.env.TENCENTCLOUD_APPID,"cloud.availability_zone":process.env.TENCENTCLOUD_ZONE}:process.env.NETLIFY?{"cloud.provider":"netlify"}:process.env.FLY_REGION?{"cloud.provider":"fly.io","cloud.region":process.env.FLY_REGION}:process.env.DYNO?{"cloud.provider":"heroku"}:void 0}n();import{readFile as Ho}from"fs";var xg=new x(100),Ro=7,jl="ContextLines";function No(I){return new Promise((g,l)=>{Ho(I,"utf8",(C,o)=>{C?l(C):g(o)})})}var Ko=(I={})=>{let g=I.frameContextLines!==void 0?I.frameContextLines:Ro;return{name:jl,setupOnce(){},processEvent(l){return vo(l,g)}}},hI=Ko,Ml=X(jl,hI);async function vo(I,g){let l={},C=[];if(g>0&&t([I,"access",o=>o.exception,"optionalAccess",o=>o.values])){for(let o of I.exception.values)if(t([o,"access",e=>e.stacktrace,"optionalAccess",e=>e.frames]))for(let e=o.stacktrace.frames.length-1;e>=0;e--){let d=o.stacktrace.frames[e];d.filename&&!l[d.filename]&&!xg.get(d.filename)&&(C.push(Jo(d.filename)),l[d.filename]=1)}}if(C.length>0&&await Promise.all(C),g>0&&t([I,"access",o=>o.exception,"optionalAccess",o=>o.values]))for(let o of I.exception.values)o.stacktrace&&o.stacktrace.frames&&await Fo(o.stacktrace.frames,g);return I}function Fo(I,g){for(let l of I)if(l.filename&&l.context_line===void 0){let C=xg.get(l.filename);if(C)try{ol(C,l,g)}catch{}}}async function Jo(I){let g=xg.get(I);if(g===null)return null;if(g!==void 0)return g;let l=null;try{l=(await No(I)).split(`
`)}catch{}return xg.set(I,l),l}n();n();var j=typeof __SENTRY_DEBUG__>"u"||__SENTRY_DEBUG__;n();import{URL as yI}from"url";function Dl(I){let{protocol:g,hostname:l,port:C}=ql(I),o=I.path?I.path:"/";return`${g}//${l}${C}${o}`}function XI(I){let{protocol:g,hostname:l,port:C}=ql(I),o=I.pathname||"/",e=I.auth?So(I.auth):"";return`${g}//${e}${l}${C}${o}`}function So(I){let[g,l]=I.split(":");return`${g?"[Filtered]":""}:${l?"[Filtered]":""}@`}function YI(I,g,l){if(!I)return I;let[C,o]=I.split(" ");return g.host&&!g.protocol&&(g.protocol=t([l,"optionalAccess",e=>e.agent,"optionalAccess",e=>e.protocol]),o=XI(g)),t([o,"optionalAccess",e=>e.startsWith,"call",e=>e("///")])&&(o=o.slice(2)),`${C} ${o}`}function Ol(I){let g={protocol:I.protocol,hostname:typeof I.hostname=="string"&&I.hostname.startsWith("[")?I.hostname.slice(1,-1):I.hostname,hash:I.hash,search:I.search,pathname:I.pathname,path:`${I.pathname||""}${I.search||""}`,href:I.href};return I.port!==""&&(g.port=Number(I.port)),(I.username||I.password)&&(g.auth=`${I.username}:${I.password}`),g}function Pl(I,g){let l,C;if(typeof g[g.length-1]=="function"&&(l=g.pop()),typeof g[0]=="string")C=Ol(new yI(g[0]));else if(g[0]instanceof yI)C=Ol(g[0]);else{C=g[0];try{let o=new yI(C.path||"",`${C.protocol||"http:"}//${C.hostname}`);C={pathname:o.pathname,search:o.search,hash:o.hash,...C}}catch{}}return g.length===2&&(C={...C,...g[1]}),C.protocol===void 0&&(S.major>8?C.protocol=t([t([I,"optionalAccess",o=>o.globalAgent]),"optionalAccess",o=>o.protocol])||t([C.agent,"optionalAccess",o=>o.protocol])||t([C._defaultAgent,"optionalAccess",o=>o.protocol]):C.protocol=t([C.agent,"optionalAccess",o=>o.protocol])||t([C._defaultAgent,"optionalAccess",o=>o.protocol])||t([t([I,"optionalAccess",o=>o.globalAgent]),"optionalAccess",o=>o.protocol])),l?[C,l]:[C]}function ql(I){let g=I.protocol||"",l=I.hostname||I.host||"",C=!I.port||I.port===80||I.port===443||/^(.*):(\d+)$/.test(l)?"":`:${I.port}`;return{protocol:g,hostname:l,port:C}}var wo=(I={})=>{let{breadcrumbs:g,tracing:l,shouldCreateSpanForRequest:C}=I,o={breadcrumbs:g,tracing:l===!1?!1:$I({enableIfHasTracingEnabled:l===!0?void 0:!0,shouldCreateSpanForRequest:C})};return new cg(o)},$l=wo,cg=class I{static __initStatic(){this.id="Http"}__init(){this.name=I.id}constructor(g={}){I.prototype.__init.call(this),this._breadcrumbs=typeof g.breadcrumbs>"u"?!0:g.breadcrumbs,this._tracing=g.tracing?g.tracing===!0?{}:g.tracing:void 0}setupOnce(g,l){let C=t([l,"call",i=>i(),"access",i=>i.getClient,"call",i=>i(),"optionalAccess",i=>i.getOptions,"call",i=>i()]),o=Qo(this._tracing,C);if(!this._breadcrumbs&&!o)return;if(C&&C.instrumenter!=="sentry"){j&&V.log("HTTP Integration is skipped because of instrumenter configuration.");return}let e=Lo(o,this._tracing,C),d=t([C,"optionalAccess",i=>i.tracePropagationTargets])||t([this,"access",i=>i._tracing,"optionalAccess",i=>i.tracePropagationTargets]),Z=f("http"),G=_l(Z,this._breadcrumbs,e,d);if(Cg(Z,"get",G),Cg(Z,"request",G),S.major>8){let i=f("https"),a=_l(i,this._breadcrumbs,e,d);Cg(i,"get",a),Cg(i,"request",a)}}};cg.__initStatic();function _l(I,g,l,C){let o=new x(100),e=new x(100),d=i=>{if(l===void 0)return!0;let a=o.get(i);if(a!==void 0)return a;let m=l(i);return o.set(i,m),m},Z=i=>{if(C===void 0)return!0;let a=e.get(i);if(a!==void 0)return a;let m=Kg(i,C);return e.set(i,m),m};function G(i,a,m,B){dI().getIntegration(cg)&&_({category:"http",data:{status_code:B&&B.statusCode,...a},type:"http"},{event:i,request:m,response:B})}return function(a){return function(...B){let A=Pl(I,B),W=A[0],Y=Dl(W),p=XI(W),w=y();if(bg(p,w))return a.apply(I,A);let z=J(),k=$(),D=tg(),F=ko(p,W),u=d(Y)?t([D,"optionalAccess",h=>h.startChild,"call",h=>h({op:"http.client",origin:"auto.http.node.http",description:`${F["http.method"]} ${F.url}`,data:F})]):void 0;if(w&&Z(Y)){let{traceId:h,spanId:P,sampled:Dg,dsc:SC}={...k.getPropagationContext(),...z.getPropagationContext()},wC=u?dg(u):vg(h,P,Dg),zC=eg(SC||(u?Zg(u):Fg(h,w,z)));zo(W,p,wC,zC)}else j&&V.log(`[Tracing] Not adding sentry-trace header to outgoing request (${p}) due to mismatching tracePropagationTargets option.`);return a.apply(I,A).once("response",function(h){let P=this;g&&G("response",F,P,h),u&&(h.statusCode&&gg(u,h.statusCode),u.updateName(YI(lI(u).description||"",W,P)||""),u.end())}).once("error",function(){let h=this;g&&G("error",F,h),u&&(gg(u,500),u.updateName(YI(lI(u).description||"",W,h)||""),u.end())})}}}function zo(I,g,l,C){(I.headers||{})["sentry-trace"]||(j&&V.log(`[Tracing] Adding sentry-trace header ${l} to outgoing request to "${g}": `),I.headers={...I.headers,"sentry-trace":l,...C&&C.length>0&&{baggage:Uo(I,C)}})}function ko(I,g){let l=g.method||"GET",C={url:I,"http.method":l};return g.hash&&(C["http.fragment"]=g.hash.substring(1)),g.search&&(C["http.query"]=g.search.substring(1)),C}function Uo(I,g){if(!I.headers||!I.headers.baggage)return g;if(g){if(Array.isArray(I.headers.baggage))return[...I.headers.baggage,g]}else return I.headers.baggage;return[I.headers.baggage,g]}function Qo(I,g){return I===void 0?!1:I.enableIfHasTracingEnabled?Sg(g):!0}function Lo(I,g,l){return I?t([g,"optionalAccess",o=>o.shouldCreateSpanForRequest])||t([l,"optionalAccess",o=>o.shouldCreateSpanForRequest]):()=>!1}n();n();n();function IC(I,g,l){let C=0,o=5,e=0;return setInterval(()=>{e===0?C>I&&(o*=2,l(o),o>86400&&(o=86400),e=o):(e-=1,e===0&&g()),C=0},1e3).unref(),()=>{C+=1}}function gC(I){return I!==void 0&&(I.length===0||I==="?"||I==="<anonymous>")}function lC(I,g){return I===g||gC(I)&&gC(g)}function HI(I){if(I!==void 0)return I.slice(-10).reduce((g,l)=>`${g},${l.function},${l.lineno},${l.colno}`,"")}function CC(I,g){if(g!==void 0)return HI(I(g,1))}function oC(I){let g=[],l=!1;function C(d){g=[],!l&&(l=!0,I(d))}g.push(C);function o(d){g.push(d)}function e(d){let Z=g.pop()||C;try{Z(d)}catch{C(d)}}return{add:o,next:e}}var RI=class{constructor(){let{Session:g}=f("inspector");this._session=new g}configureAndConnect(g,l){this._session.connect(),this._session.on("Debugger.paused",C=>{g(C,()=>{this._session.post("Debugger.resume")})}),this._session.post("Debugger.enable"),this._session.post("Debugger.setPauseOnExceptions",{state:l?"all":"uncaught"})}setPauseOnExceptions(g){this._session.post("Debugger.setPauseOnExceptions",{state:g?"all":"uncaught"})}getLocalVariables(g,l){this._getProperties(g,C=>{let{add:o,next:e}=oC(l);for(let d of C)if(t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.objectId])&&t([d,"optionalAccess",Z=>Z.value,"access",Z=>Z.className])==="Array"){let Z=d.value.objectId;o(G=>this._unrollArray(Z,d.name,G,e))}else if(t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.objectId])&&t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.className])==="Object"){let Z=d.value.objectId;o(G=>this._unrollObject(Z,d.name,G,e))}else(t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.value])!=null||t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.description])!=null)&&o(Z=>this._unrollOther(d,Z,e));e({})})}_getProperties(g,l){this._session.post("Runtime.getProperties",{objectId:g,ownProperties:!0},(C,o)=>{l(C?[]:o.result)})}_unrollArray(g,l,C,o){this._getProperties(g,e=>{C[l]=e.filter(d=>d.name!=="length"&&!isNaN(parseInt(d.name,10))).sort((d,Z)=>parseInt(d.name,10)-parseInt(Z.name,10)).map(d=>t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.value])),o(C)})}_unrollObject(g,l,C,o){this._getProperties(g,e=>{C[l]=e.map(d=>[d.name,t([d,"optionalAccess",Z=>Z.value,"optionalAccess",Z=>Z.value])]).reduce((d,[Z,G])=>(d[Z]=G,d),{}),o(C)})}_unrollOther(g,l,C){t([g,"optionalAccess",o=>o.value,"optionalAccess",o=>o.value])!=null?l[g.name]=g.value.value:t([g,"optionalAccess",o=>o.value,"optionalAccess",o=>o.description])!=null&&t([g,"optionalAccess",o=>o.value,"optionalAccess",o=>o.type])!=="function"&&(l[g.name]=`<${g.value.description}>`),C(l)}};function fo(){try{return new RI}catch{return}}var cC="LocalVariables",xo=(I={},g=fo())=>{let l=new x(20),C,o=!1;function e(G,{params:{reason:i,data:a,callFrames:m}},B){if(i!=="exception"&&i!=="promiseRejection"){B();return}t([C,"optionalCall",p=>p()]);let A=CC(G,t([a,"optionalAccess",p=>p.description]));if(A==null){B();return}let{add:W,next:Y}=oC(p=>{l.set(A,p),B()});for(let p=0;p<Math.min(m.length,5);p++){let{scopeChain:w,functionName:z,this:k}=m[p],D=w.find(u=>u.type==="local"),F=k.className==="global"||!k.className?z:`${k.className}.${z}`;if(t([D,"optionalAccess",u=>u.object,"access",u=>u.objectId])===void 0)W(u=>{u[p]={function:F},Y(u)});else{let u=D.object.objectId;W(h=>t([g,"optionalAccess",P=>P.getLocalVariables,"call",P=>P(u,Dg=>{h[p]={function:F,vars:Dg},Y(h)})]))}}Y([])}function d(G){let i=HI(t([G,"optionalAccess",B=>B.stacktrace,"optionalAccess",B=>B.frames]));if(i===void 0)return;let a=l.remove(i);if(a===void 0)return;let m=(t([G,"access",B=>B.stacktrace,"optionalAccess",B=>B.frames])||[]).filter(B=>B.function!=="new Promise");for(let B=0;B<m.length;B++){let A=m.length-B-1;if(!m[A]||!a[B])break;a[B].vars===void 0||m[A].in_app===!1||!lC(m[A].function,a[B].function)||(m[A].vars=a[B].vars)}}function Z(G){for(let i of t([G,"optionalAccess",a=>a.exception,"optionalAccess",a=>a.values])||[])d(i);return G}return{name:cC,setupOnce(){let G=y(),i=t([G,"optionalAccess",a=>a.getOptions,"call",a=>a()]);if(g&&t([i,"optionalAccess",a=>a.includeLocalVariables])){if(S.major<18){V.log("The `LocalVariables` integration is only supported on Node >= v18.");return}let m=I.captureAllExceptions!==!1;if(g.configureAndConnect((B,A)=>e(i.stackParser,B,A),m),m){let B=I.maxExceptionsPerSecond||50;C=IC(B,()=>{V.log("Local variables rate-limit lifted."),t([g,"optionalAccess",A=>A.setPauseOnExceptions,"call",A=>A(!0)])},A=>{V.log(`Local variables rate-limit exceeded. Disabling capturing of caught exceptions for ${A} seconds.`),t([g,"optionalAccess",W=>W.setPauseOnExceptions,"call",W=>W(!1)])})}o=!0}},processEvent(G){return o?Z(G):G},_getCachedFramesCount(){return l.size},_getFirstCachedFrame(){return l.values()[0]}}},NI=xo,eC=X(cC,NI);var dC=eC,ZC=NI;n();import{existsSync as To,readFileSync as Eo}from"fs";import{dirname as jo,join as Mo}from"path";var KI,bC="Modules";function Oo(){try{return f.cache?Object.keys(f.cache):[]}catch{return[]}}function Do(){let I=f.main&&f.main.paths||[],g=Oo(),l={},C={};return g.forEach(o=>{let e=o,d=()=>{let Z=e;if(e=jo(Z),!e||Z===e||C[Z])return;if(I.indexOf(e)<0)return d();let G=Mo(Z,"package.json");if(C[Z]=!0,!To(G))return d();try{let i=JSON.parse(Eo(G,"utf8"));l[i.name]=i.version}catch{}};d()}),l}function Po(){return KI||(KI=Do()),KI}var qo=()=>({name:bC,setupOnce(){},processEvent(I){return I.modules={...I.modules,...Po()},I}}),vI=qo,nC=X(bC,vI);n();n();var _o=2e3;function Vg(I){lg(()=>{console.error(I)});let g=y();g===void 0&&(j&&V.warn("No NodeClient was defined, we are exiting the process now."),global.process.exit(1));let l=g.getOptions(),C=l&&l.shutdownTimeout&&l.shutdownTimeout>0&&l.shutdownTimeout||_o;g.close(C).then(o=>{o||j&&V.warn("We reached the timeout for emptying the request buffer, still exiting now!"),global.process.exit(1)},o=>{j&&V.error(o)})}var iC="OnUncaughtException",$o=(I={})=>{let g={exitEvenIfOtherHandlersAreRegistered:!0,...I};return{name:iC,setupOnce(){},setup(l){global.process.on("uncaughtException",gc(l,g))}}},FI=$o,GC=X(iC,FI);function gc(I,g){let C=!1,o=!1,e=!1,d,Z=I.getOptions();return Object.assign(G=>{let i=Vg;g.onFatalError?i=g.onFatalError:Z.onFatalError&&(i=Z.onFatalError);let m=global.process.listeners("uncaughtException").reduce((A,W)=>W.name==="domainUncaughtExceptionClear"||W.tag&&W.tag==="sentry_tracingErrorCallback"||W._errorHandler?A:A+1,0)===0,B=g.exitEvenIfOtherHandlersAreRegistered||m;C?B&&(e?(j&&V.warn("uncaught exception after calling fatal error shutdown callback - this is bad! forcing shutdown"),Vg(G)):o||(o=!0,setTimeout(()=>{e||(e=!0,i(d,G))},2e3))):(d=G,C=!0,y()===I&&og(G,{originalException:G,captureContext:{level:"fatal"},mechanism:{handled:!1,type:"onuncaughtexception"}}),!e&&B&&(e=!0,i(G)))},{_errorHandler:!0})}n();var aC="OnUnhandledRejection",Ic=(I={})=>{let g=I.mode||"warn";return{name:aC,setupOnce(){},setup(l){global.process.on("unhandledRejection",lc(l,{mode:g}))}}},JI=Ic,BC=X(aC,JI);function lc(I,g){return function(C,o){y()===I&&(og(C,{originalException:o,captureContext:{extra:{unhandledPromiseRejection:!0}},mechanism:{handled:!1,type:"onunhandledrejection"}}),Cc(C,g))}}function Cc(I,g){let l="This error originated either by throwing inside of an async function without a catch block, or by rejecting a promise which was not handled with .catch(). The promise rejected with the reason:";g.mode==="warn"?lg(()=>{console.warn(l),console.error(I&&I.stack?I.stack:I)}):g.mode==="strict"&&(lg(()=>{console.warn(l)}),Vg(I))}n();import*as oc from"http";import{URL as cc}from"url";var tC="Spotlight",ec=(I={})=>{let g={sidecarUrl:I.sidecarUrl||"http://localhost:8969/stream"};return{name:tC,setupOnce(){},setup(l){typeof process=="object"&&process.env&&V.warn("[Spotlight] It seems you're not in dev mode. Do you really want to have Spotlight enabled?"),Zc(l,g)}}},dc=ec,mC=X(tC,dc);function Zc(I,g){let l=bc(g.sidecarUrl);if(!l)return;let C=0;if(typeof I.on!="function"){V.warn("[Spotlight] Cannot connect to spotlight due to missing method on SDK client (`client.on`)");return}I.on("beforeEnvelope",o=>{if(C>3){V.warn("[Spotlight] Disabled Sentry -> Spotlight integration due to too many failed requests");return}let e=nl(o),Z=nc()({method:"POST",path:l.pathname,hostname:l.hostname,port:l.port,headers:{"Content-Type":"application/x-sentry-envelope"}},G=>{G.on("data",()=>{}),G.on("end",()=>{}),G.setEncoding("utf8")});Z.on("error",()=>{C++,V.warn("[Spotlight] Failed to send envelope to Spotlight Sidecar")}),Z.write(e),Z.end()})}function bc(I){try{return new cc(`${I}`)}catch{V.warn(`[Spotlight] Invalid sidecar URL: ${I}`);return}}function nc(){let{request:I}=oc;return ic(I)?I.__sentry_original__:I}function ic(I){return"__sentry_original__"in I}n();var rg;(function(I){let g="undici:request:create";I.RequestCreate=g;let l="undici:request:headers";I.RequestEnd=l;let C="undici:request:error";I.RequestError=C})(rg||(rg={}));var Gc=I=>new ng(I),sC=Gc,ng=class I{static __initStatic(){this.id="Undici"}__init(){this.name=I.id}__init2(){this._createSpanUrlMap=new x(100)}__init3(){this._headersUrlMap=new x(100)}constructor(g={}){I.prototype.__init.call(this),I.prototype.__init2.call(this),I.prototype.__init3.call(this),I.prototype.__init4.call(this),I.prototype.__init5.call(this),I.prototype.__init6.call(this),this._options={breadcrumbs:g.breadcrumbs===void 0?!0:g.breadcrumbs,tracing:g.tracing,shouldCreateSpanForRequest:g.shouldCreateSpanForRequest}}setupOnce(g){if(S.major<16)return;let l;try{l=f("diagnostics_channel")}catch{}!l||!l.subscribe||(l.subscribe(rg.RequestCreate,this._onRequestCreate),l.subscribe(rg.RequestEnd,this._onRequestEnd),l.subscribe(rg.RequestError,this._onRequestError))}_shouldCreateSpan(g){if(this._options.tracing===!1||this._options.tracing===void 0&&!Sg())return!1;if(this._options.shouldCreateSpanForRequest===void 0)return!0;let l=this._createSpanUrlMap.get(g);if(l!==void 0)return l;let C=this._options.shouldCreateSpanForRequest(g);return this._createSpanUrlMap.set(g,C),C}__init4(){this._onRequestCreate=g=>{if(!t([y,"call",m=>m(),"optionalAccess",m=>m.getIntegration,"call",m=>m(I)]))return;let{request:l}=g,C=l.origin?l.origin.toString()+l.path:l.path,o=y();if(!o||bg(C,o)||l.__sentry_span__!==void 0)return;let e=o.getOptions(),d=J(),Z=$(),G=tg(),i=this._shouldCreateSpan(C)?Bc(G,l,C):void 0;if(i&&(l.__sentry_span__=i),(m=>{if(e.tracePropagationTargets===void 0)return!0;let B=this._headersUrlMap.get(m);if(B!==void 0)return B;let A=Kg(m,e.tracePropagationTargets);return this._headersUrlMap.set(m,A),A})(C)){let{traceId:m,spanId:B,sampled:A,dsc:W}={...Z.getPropagationContext(),...d.getPropagationContext()},Y=i?dg(i):vg(m,B,A),p=eg(W||(i?Zg(i):Fg(m,o,d)));ac(l,Y,p)}}}__init5(){this._onRequestEnd=g=>{if(!t([y,"call",d=>d(),"optionalAccess",d=>d.getIntegration,"call",d=>d(I)]))return;let{request:l,response:C}=g,o=l.origin?l.origin.toString()+l.path:l.path;if(bg(o,y()))return;let e=l.__sentry_span__;e&&(gg(e,C.statusCode),e.end()),this._options.breadcrumbs&&_({category:"http",data:{method:l.method,status_code:C.statusCode,url:o},type:"http"},{event:"response",request:l,response:C})}}__init6(){this._onRequestError=g=>{if(!t([y,"call",e=>e(),"optionalAccess",e=>e.getIntegration,"call",e=>e(I)]))return;let{request:l}=g,C=l.origin?l.origin.toString()+l.path:l.path;if(bg(C,y()))return;let o=l.__sentry_span__;o&&(o.setStatus("internal_error"),o.end()),this._options.breadcrumbs&&_({category:"http",data:{method:l.method,url:C},level:"error",type:"http"},{event:"error",request:l})}}};ng.__initStatic();function ac(I,g,l){let C;Array.isArray(I.headers)?C=I.headers.some(o=>o==="sentry-trace"):C=I.headers.split(`\r
`).some(e=>e.startsWith("sentry-trace:")),!C&&(I.addHeader("sentry-trace",g),l&&I.addHeader("baggage",l))}function Bc(I,g,l){let C=dl(l),o=g.method||"GET",e={"http.method":o};return C.search&&(e["http.query"]=C.search),C.hash&&(e["http.fragment"]=C.hash),t([I,"optionalAccess",d=>d.startChild,"call",d=>d({op:"http.client",origin:"auto.http.node.undici",description:`${o} ${Zl(C)}`,data:e})])}n();import{posix as tc,sep as mc}from"path";function AC(I){return I.replace(/^[A-Z]:/,"").replace(/\\/g,"/")}function Tg(I=process.argv[1]?el(process.argv[1]):process.cwd(),g=mc==="\\"){let l=g?AC(I):I;return C=>{if(!C)return;let o=g?AC(C):C,{dir:e,base:d,ext:Z}=tc.parse(o);(Z===".js"||Z===".mjs"||Z===".cjs")&&(d=d.slice(0,Z.length*-1)),e||(e=".");let G=e.lastIndexOf("/node_modules");if(G>-1)return`${e.slice(G+14).replace(/\//g,".")}:${d}`;if(e.startsWith(l)){let i=e.slice(l.length+1).replace(/\//g,".");return i&&(i+=":"),i+=d,i}return d}}var WC=[nI(),iI(),GI(),bI(),rI(),$l(),sC(),FI(),JI(),hI(),ZC(),pI(),vI()];function SI(I){let g=il(),l=t([g,"access",C=>C.__SENTRY__,"optionalAccess",C=>C.integrations])||[];return[...WC,...l]}var Eg=gl(Il(Tg()));n();import{URL as Wc}from"url";n();var uC="";var uc=50,Vc=5e3;function wI(I,...g){V.log(`[ANR] ${I}`,...g)}function rc(){return gI}function pc(){let I=eI().getScopeData();return oI(I,$().getScopeData()),oI(I,J().getScopeData()),I.attachments=[],I.eventProcessors=[],I}function hc(){return cl(module,"worker_threads")}async function yc(I){let g={message:"ANR"},l={};for(let C of I.getEventProcessors()){if(g===null)break;g=await C(g,l)}return t([g,"optionalAccess",C=>C.contexts])||{}}var VC="Anr",Xc=(I={})=>{if(S.major<16||S.major===16&&S.minor<17)throw new Error("ANR detection requires Node 16.17.0 or later");let g,l,C=rc();return C.__SENTRY_GET_SCOPES__=pc,{name:VC,setupOnce(){},startWorker:()=>{g||l&&(g=Hc(l,I))},stopWorker:()=>{g&&g.then(o=>{o(),g=void 0})},setup(o){l=o,setImmediate(()=>this.startWorker())}}},Yc=Xc,rC=X(VC,Yc);async function Hc(I,g){let l=I.getDsn();if(!l)return()=>{};let C=await yc(I);II([C,"access",a=>a.app,"optionalAccess",a=>delete a.app_memory]),II([C,"access",a=>a.device,"optionalAccess",a=>delete a.free_memory]);let o=I.getOptions(),e=I.getSdkMetadata()||{};e.sdk&&(e.sdk.integrations=o.integrations.map(a=>a.name));let d={debug:V.isEnabled(),dsn:l,environment:o.environment||"production",release:o.release,dist:o.dist,sdkMetadata:e,appRootPath:g.appRootPath,pollInterval:g.pollInterval||uc,anrThreshold:g.anrThreshold||Vc,captureStackTrace:!!g.captureStackTrace,staticTags:g.staticTags||{},contexts:C};if(d.captureStackTrace){let a=f("inspector");a.url()||a.open(0)}let{Worker:Z}=hc(),G=new Z(new Wc(`data:application/javascript;base64,${uC}`),{workerData:d});process.on("exit",()=>{G.terminate()});let i=setInterval(()=>{try{let a=J().getSession(),m=a?{...a,toJSON:void 0}:void 0;G.postMessage({session:m})}catch{}},d.pollInterval);return i.unref(),G.on("message",a=>{a==="session-ended"&&(wI("ANR event sent from ANR worker. Clearing session in this thread."),J().setSession(void 0))}),G.once("error",a=>{clearInterval(i),wI("ANR worker error",a)}),G.once("exit",a=>{clearInterval(i),wI("ANR worker exit",a)}),G.unref(),()=>{G.terminate(),clearInterval(i)}}var zI={};EI(zI,{Anr:()=>rC,Console:()=>Ll,Context:()=>El,ContextLines:()=>Ml,Hapi:()=>yC,Http:()=>cg,LocalVariables:()=>dC,Modules:()=>nC,OnUncaughtException:()=>GC,OnUnhandledRejection:()=>BC,RequestData:()=>Bl,Spotlight:()=>mC,Undici:()=>ng});n();n();function pC(I){return I&&I.statusCode!==void 0}function Rc(I){return I&&I.error!==void 0}function Nc(I){og(I,{mechanism:{type:"hapi",handled:!1,data:{function:"hapiErrorPlugin"}}})}var Kc={name:"SentryHapiErrorPlugin",version:Jg,register:async function(I){I.events.on("request",(l,C)=>{let o=Bg();Rc(C)&&Nc(C.error),o&&(o.setStatus("internal_error"),o.end())})}},vc={name:"SentryHapiTracingPlugin",version:Jg,register:async function(I){let g=I;g.ext("onPreHandler",(l,C)=>{let o=ZI({sentryTrace:l.headers["sentry-trace"]||void 0,baggage:l.headers.baggage||void 0},e=>CI({...e,op:"hapi.request",name:l.route.path,description:`${l.route.method} ${l.path}`}));return J().setSpan(o),C.continue}),g.ext("onPreResponse",(l,C)=>{let o=Bg();if(l.response&&pC(l.response)&&o){let e=l.response;e.header("sentry-trace",dg(o));let d=eg(Zg(o));d&&e.header("baggage",d)}return C.continue}),g.ext("onPostHandler",(l,C)=>{let o=Bg();return o&&(l.response&&pC(l.response)&&gg(o,l.response.statusCode),o.end()),C.continue})}},hC="Hapi",Fc=(I={})=>{let g=I.server;return{name:hC,setupOnce(){g&&Cg(g,"start",l=>async function(){return await this.register(vc),await this.register(Kc),l.apply(this)})}}},Jc=Fc,yC=X(hC,Jc);var kI={};EI(kI,{Apollo:()=>rl,Express:()=>ml,GraphQL:()=>Vl,Mongo:()=>Wl,Mysql:()=>Al,Postgres:()=>sl,Prisma:()=>ul});n();var en=Tg(),dn={...tl,...zI,...kI};var UI=H(qg(),1);import Sc from"node:events";var jg=(0,ig.variant)({Reported:(0,ig.fields)(),Ignored:{}}),wc=(I,g,{data:l}={})=>{if(!Rg(g))return jg.Ignored();let C=(0,ig.isType)(g.cliCode,DI.UnknownFailure)?g.cliCode.error:g,o=I.captureException(C,{data:l,captureContext:e=>{let d=mg(C);return d&&(e.setTag("requestId",d.requestId),e.setContext("requestInfo",zg(d))),C instanceof Error&&e.setContext("error details",{info:UI.default.info(C),fullStack:UI.default.fullStack(C)}),e}});return jg.Reported({eventId:o})};function XC({dsn:I,release:g}){let{sentryDsn:l}=Hg(),C=new Ag({enabled:l!==void 0||!0,dsn:l??I,release:g,integrations:SI({}).filter(d=>d.name!=="OnUnhandledRejection"&&d.name!=="OnUncaughtException"&&d.name!=="Modules"),stackParser:Eg,transport:fg,beforeSend(d){return delete d.server_name,d},normalizeDepth:10}),o=new cI;o.setClient(C),C.init();let e=new Sc;return{on:e.on.bind(e),reportError:(d,{data:Z}={})=>{let G=Ng(d),i=wc(o,G,{data:Z});return e.emit("errorReported",G),i},reportUserFeedback:async({userId:d,eventId:Z,feedback:G})=>{let{BrowserClient:i}=await import("./esm-J4ZWVMEG.js");new i({enabled:C.getOptions().enabled,dsn:C.getOptions().dsn,integrations:[],transport:fg,stackParser:Eg}).captureUserFeedback({event_id:Z,name:"",email:`${d}@wix.com`,comments:v`
          ---
          user: https://bo.wix.com/um/users/${d}
          ---
          ${G}
        `})},setContext:(d,Z)=>o.setContext(d,Z),setUser:d=>o.setUser(d),getUser:()=>{let d=o.getUser();if(typeof d?.id=="string")return{id:d.id}}}}n();var hg=H(U(),1),xI=H(Xg(),1);import{stdin as Qc}from"node:process";n();var Mg=H(Xg(),1);import{join as YC}from"node:path";var pg=(0,Mg.variant)({Exists:(0,Mg.fields)(),NotExists:{}}),zc=yg.object({data:yg.unknown(),createdAt:yg.number()}),kc=(I,g)=>Date.now()-I.valueOf()>g*60*60*1e3;function Hn(I,g){let l=Hg();return l.fsCacheDir?YC(l.fsCacheDir,g):YC(I,g)}function HC({filePath:I,ttlInHours:g=24*7}){return{get:async()=>{try{if(await yl(I)){let e=zc.parse(await hl(I));if(!kc(e.createdAt,g))return pg.Exists({data:e.data})}return pg.NotExists()}catch(o){throw new _g({code:$g.FailedToReadCache({path:I}),cause:o})}},set:async o=>{try{return await pl(I,{data:o,createdAt:Date.now()}),pg.Exists({data:o})}catch(e){throw new _g({code:$g.FailedToWriteCache({path:I}),cause:e})}}}}n();var K=H(U(),1);n();var QI=H(U(),1);var Og=()=>QI.default.createElement(Xl,{i18nKey:"user_feedback.contact_us",components:[QI.default.createElement(Yg,{url:"https://discord.gg/devs-on-wix"})]});var RC=({onSubmit:I})=>{let{t:g}=Yl(),[l,C]=(0,K.useState)(!1),[o,e]=(0,K.useState)(),d=(0,K.useCallback)(G=>{e(G),G||C(!0)},[]),Z=(0,K.useCallback)(G=>{C(!0),I({feedback:G})},[I]);return K.default.createElement(K.default.Fragment,null,K.default.createElement(Q,{flexDirection:"column",marginTop:1,marginBottom:1},K.default.createElement(s,null,g("user_feedback.title")),K.default.createElement(s,{skin:"secondary"},g("user_feedback.subtitle"))),K.default.createElement(MI,{label:g("user_feedback.consent.label"),onSubmit:d}),o===!0&&K.default.createElement(OI,{label:g("user_feedback.comments.label"),onSubmit:Z}),l&&K.default.createElement(Q,{flexDirection:"column",marginTop:1},K.default.createElement(s,null,g(o?"user_feedback.submitted":"user_feedback.declined")),K.default.createElement(Og,null)))};n();var LI=H(U(),1);var NC=()=>LI.default.createElement(Q,{flexDirection:"column",marginTop:1},LI.default.createElement(Og,null));var fI={"user_feedback.title":"Your experience matters to us. Would you like to submit feedback regarding this incident?","user_feedback.subtitle":"We may contact you by email in case we need more details.","user_feedback.consent.label":"Submit feedback?","user_feedback.comments.label":"Feedback (optional)","user_feedback.submitted":"Thank you for your cooperation in helping us resolve this issue promptly.","user_feedback.declined":"Thank you for your feedback.","user_feedback.contact_us":"Reach us in Discord in case you have more questions: <0 />"};async function KC({cachePath:I,reportResult:g,errorReporter:l}){if(!Qc.isTTY||!I||!(0,xI.isType)(g,jg.Reported))return;let C=HC({filePath:I,ttlInHours:1}),o=await C.get();if((0,xI.isType)(o,pg.Exists))return;let e=l.getUser();e?await PI(hg.default.createElement(aI,{messages:fI},hg.default.createElement(RC,{onSubmit:({feedback:d})=>{l.reportUserFeedback({userId:e.id,eventId:g.eventId,feedback:d})}}))):qI(hg.default.createElement(aI,{messages:fI},hg.default.createElement(NC,null))),await C.set({reported:!0})}function Lc({debugOutputPath:I,userFeedbackCachePath:g,...l}){let C=XC(l),o=function({error:d,data:Z}){let G=C.reportError(d,{data:Z});_I(vC.default.createElement(WI,{error:d})),tI(I,d),TI.exitCode=1,KC({reportResult:G,errorReporter:C,cachePath:g}).catch(i=>{tI(I,i)})};return{handler:o,errorReporter:C,registerToProcessErrors:function(){TI.on("uncaughtException",d=>{o({error:d,data:{mechanism:{handled:!1,type:"uncaughtException"}}})}),TI.on("unhandledRejection",d=>{d instanceof Error&&d.message.startsWith("Build failed with")&&d.stack?.includes("esbuild")||o({error:d,data:{mechanism:{handled:!1,type:"unhandledRejection"}}})})}}}n();var Gg=H(U(),1),FC=(0,Gg.createContext)(null),fc=I=>Gg.default.createElement(FC.Provider,{...I}),xc=()=>{let I=(0,Gg.useContext)(FC);if(I===null)throw new Error("useErrorReporter() cannot be used without an <ErrorReporterProvider>");return I};n();n();var ag=H(U(),1),JC=(0,ag.createContext)(null),Tc=function({children:I,value:g}){return ag.default.createElement(JC.Provider,{value:g},I)},Ec=()=>{let I=(0,ag.useContext)(JC);if(I===null)throw new Error("useBiLogger() cannot be used without an <BiProvider>");return I};n();export{mg as a,tI as b,WI as c,Hn as d,HC as e,Lc as f,fc as g,xc as h,Tc as i,Ec as j};
/*! Bundled license information:

@sentry/node/esm/integrations/anr/worker-script.js:
  (*! @sentry/node 7.118.0 (090af74) | https://github.com/getsentry/sentry-javascript *)
*/
//# sourceMappingURL=chunk-J7ENM3TA.js.map